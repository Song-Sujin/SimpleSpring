...
<body>
	...
	<!-- 검색폼 -->
	<form action="${pageContext.request.contextPath }/department/list.do" method="get">
		<label for="keyword">검색어: </label>
		<input type="search" name="keyword" id="keyword" placeholder="학과명 or 위치 검색"
		value="${keyword }"/><!-- 검색어에 대한 상태유지 처리 -->
	...
	<c:choose>
		...
		<c:when test="${pageData.prevPage > 0 }">
			<%-- 이동할 URL 생성 --%>
			<c:url value="/department/list.do" var="prevPageUrl"><%-- 검색 결과에 대한 페이지 번호를 대비하여 검색어와 페이지 번호를 모두 상태 유지 처리 --%>
	...
	<c:forEach var="i" begin="${pageData.startPage }" end="${pageDat.endPage }" varStatus="status">
		<%-- 이동할 URL 생성 --%>
