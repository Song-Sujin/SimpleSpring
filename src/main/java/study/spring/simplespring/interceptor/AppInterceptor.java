...
// Interceptor 클래스 정의. 
// 컨트롤러의 실행 요청을 중간에 가로채서 어떤 일을 수행하기 하기
// ex. 컨트롤러의 실행 직전, 실핼 직후를 로그로 남겨 컨텐츠의 인기도 측정
@Slf4j
public class AppInterceptor extends HandlerInterceptorAdapter
{
	...
	/*
	 * 자주사용 Controller 실행 요청 전에 수행되는 메서드 클라이언트의 요청을 컨트롤러에 전달 하기 전에 호출된다. return 값으로
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception
	{
		...
		String browserStr = String.format("- Brower: {family=%s, patch=%s, major=%s, minor=%s}", browser.get("family"),
	...
	/*
	 * 자주사용 view 단으로 forward 되기 전에 수행. 컨트롤러 로직이 실행된 이후 호출된다. 컨트롤러 단에서 에러 발생 시 해당
	...
	/*
	 * Servlet 3.0부터 비동기 요청이 가능해짐에 따라 비동기 요청 시 PostHandle과 afterCompletion메서드를 수행하지
