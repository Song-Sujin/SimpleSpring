...
public class SessionController
{
	...
	*/
	...
	public String sessionWrite(Model model, HttpServletRequest request)
	{
		...
		// 세션값은 request 내장 객체를 통해서 HttpSession객체를 생성해야 접근할 수 있다.
		// --> Servlet과 동일함
	...
	--> 이 페이지를 "/session/save.do" URL 에 POST 방식으로 노출시킨다.
	*/
	@RequestMapping(value="/session/save.do", method=RequestMethod.POST)
	public String sessionSave(HttpServletRequest request,	// session을 정의하기 위해 request 객체를 주입받는다.
			@RequestParam(value="user_input", defaultValue="") String userInput)	// POST 파라미터를 받기 위해 sessionSave 메서드에 파라미터를 정의한다.
	{
		...
		// 3) Spring방식의 페이지 이동
		// Servlet의 response.sendRedirect(url)과 동일. "redirect:경로"형식의 문자열을 리턴하면 페이지 강제 이동
		// --> "/"부터 시작할 경우 ContextPath는 자동으로 앞에 추가됨
